...
export default ({ oldPassword, newPassword, confirmNewPassword }) => {
    const errors = {};

    if (!oldPassword) {
        errors.oldPassword = CAN_NOT_BE_BLANK;
    }

    if (!newPassword) {
        errors.newPassword = CAN_NOT_BE_BLANK;
    } else if (newPassword.length < PASSWORD_MIN_LENGTH) {
        errors.newPassword = PASSWORD_MIN_LENGTH_NOT_REACHED;
    }
